...
namespace CLAPText2Sound
{
        ...
        std::function<void(const juce::String&)> progressCallback)
    {
        ...
        if (progressCallback)
            progressCallback("Creating embeddings for " + juce::String(allChunks.size()) + " chunks...");
        ...
        const std::vector<std::vector<float>>& embeddings) const
    {
        ...
        if (outputStream.openedOk())
        {
            ...
            outputStream.flush();
            DBG("SoundPaletteCreator: Saved " + juce::String(numEmbeddings) + " embeddings to " + embeddingsFile.getFullPathName());
            return true;
        }
        else
        {
            DBG("SoundPaletteCreator: Failed to create embeddings file");
            return false;
        }
    }
}
