...
namespace CLAPText2Sound
{
    ...
    {
    public:
        PaletteCreationWorkerThread(const juce::File& sourceFolder, int chunkSizeSeconds)
    ...
    private:
        ...
        void parseProgressMessage(const juce::String& message)
        {
            ...
            else if (message.startsWith("Creating embeddings"))
            {
                m_currentPhase = "Creating embeddings";
                // Extract chunk count from "Creating embeddings for 50 chunks..."
                auto parts = juce::StringArray::fromTokens(message, " ", "");
            ...
            else if (message.startsWith("Processing chunk "))
            {
                m_currentPhase = "Creating embeddings";
