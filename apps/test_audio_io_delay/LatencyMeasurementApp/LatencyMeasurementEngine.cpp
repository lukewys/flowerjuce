...
{
    if (testRunning.load()) return false;
    ...
    int numSamples, const juce::AudioIODeviceCallbackContext&)
{
    for (int ch = 0; ch < numOutputChannels; ++ch)
        if (outputChannelData[ch]) std::fill_n(outputChannelData[ch], numSamples, 0.0f);
...
{
    if (!measurementComplete.load()) return lastResult;
    ...
    result.peakRatio = (corrResult.correlationRMS > 1e-10) ? 
                       (corrResult.peakValue / corrResult.correlationRMS) : 0.0;
...
void LatencyMeasurementEngine::validateSignalQuality(LatencyResult& result)
{
    ...
    constexpr double minPeakSharpness = 3.0;      // Peak must be 3x neighbors
    ...
    if (result.peakSharpness < minPeakSharpness) {
        ...
        result.warningMessage = "Low signal quality - result may be inaccurate.\n\n"
            "Peak sharpness: " + juce::String(result.peakSharpness, 2) + " (need >= " + juce::String(minPeakSharpness, 1) + ")\n"
            "Estimated latency: " + juce::String(result.latencyMs, 2) + " ms (" + juce::String(result.latencySamples) + " samples)\n\n"
    ...
    const std::vector<float>& signal1, const std::vector<float>& signal2)
{
    ...
    if (!fft) fft = std::make_unique<juce::dsp::FFT>(fftOrder);
