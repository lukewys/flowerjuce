...
void MainComponent::showClickSynthWindow()
{
    if (clickSynthWindow == nullptr)
    {
        ...
        clickSynthWindow = std::make_unique<VampNet::ClickSynthWindow>(looperEngine, numTracks, &midiLearnManager);
    }
...
void MainComponent::showSamplerWindow()
{
    ...
    {
        int numTracks = static_cast<int>(tracks.size());
        samplerWindow = std::make_unique<VampNet::SamplerWindow>(looperEngine, numTracks, &midiLearnManager);
    }
