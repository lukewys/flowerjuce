...
bool LooperReadHead::advance(float wrap_pos)
{   
    static int call_count = 0;
    call_count++;
    bool is_first_call = (call_count == 1);
    ...
    if (is_first_call)
        DBG_SEGFAULT("ENTRY: LooperReadHead::advance, wrap_pos=" + juce::String(wrap_pos));
    
    // Safety check: if wrap_pos is 0 or invalid, don't advance
    if (wrap_pos < 0.0f)
    {
        juce::Logger::writeToLog("WARNING: Wrap position is 0 or invalid in advance");
        if (is_first_call)
            DBG_SEGFAULT("wrap_pos is invalid, returning false");
        return false;
    }
    ...
    bool wrapped = current_pos >= wrap_pos;
    current_pos = std::fmod(current_pos, wrap_pos);
